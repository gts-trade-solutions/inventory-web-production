...
| **WADR-026** | **Sync cursors carry a keyset position per entity, not one shared timestamp**                                | A shared position advances to the newest row anywhere, so a paging entity loses every row behind it — a first sync that silently drops items while reporting itself complete                        | One global timestamp cursor; offset paging                            |
| **WADR-027** | **All timestamps are stored as DATETIME(3), the precision JavaScript can represent**                        | MySQL will store microseconds a JS Date cannot express, so a cursor built from a row could never advance past it and the same rows returned on every pull for ever                                  | DATETIME(6) with string-encoded cursors; second precision             |
